...
function PlatformCards({ platforms, onShopNow }: { platforms: string[]; onShopNow: (platform: string) => void }) {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 12 }}>
        {platforms.map((platform) => {
          ...
          return (
            <Pressable
              key={platform}
              onPress={() => onShopNow(platform)}
              style={{ backgroundColor: style.bg, borderRadius: 16, padding: 16, width: 140, borderWidth: 1, borderColor: style.bg }}
            >
...
function CategoryRow({
  categories, selected, onSelect, onOffersHub,
}: {
  categories: string[];
  ...
  return (
    <View style={{ marginBottom: 24 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, marginBottom: 12 }}>
        ...
        <Pressable onPress={onOffersHub} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#FEF3C7', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 99 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 4 }}>
        <Pressable onPress={() => onSelect(null)} style={{ alignItems: 'center', marginRight: 12 }}>
          <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: selected === null ? INDIGO : '#EEF2FF', alignItems: 'center', justifyContent: 'center', marginBottom: 6 }}>
            <ShoppingBag size={22} color={selected === null ? '#fff' : INDIGO} />
          </View>
          <Text style={{ fontSize: 11, fontWeight: '600', color: selected === null ? INDIGO : '#6B7280' }}>All</Text>
        </Pressable>
        {categories.map((cat) => {
          const Icon = CATEGORY_MAP[cat] ?? ShoppingBag;
          const active = selected === cat;
          return (
            <Pressable key={cat} onPress={() => onSelect(active ? null : cat)} style={{ alignItems: 'center', marginRight: 12 }}>
...
function CashbackSection({ category, platforms }: { category: string; platforms: string[] }) {
  if (platforms.length === 0) return null;
  return (
    <View style={{ marginBottom: 20, paddingHorizontal: 16 }}>
      <Text style={{ fontSize: 14, fontWeight: '700', color: '#374151', marginBottom: 10 }}>
        Get Cashback on {category} buys
      </Text>
...
function TopDealsRow({ products, onPress }: { products: Product[]; onPress: (p: Product) => void }) {
  ...
  return (
    <View style={{ marginBottom: 24 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 12 }}>
        {products.map((item) => (
          <ProductCard key={item.id} product={item} onPress={onPress} featured />
        ))}
      </ScrollView>
    </View>
  );
...
export default function HomeScreen({ navigation }: Props) {
  ...
  // Category → platforms map for cashback sections
  ...
  const handleOffersHub = () => {
    // Scroll to coupons — just navigate to Deals tab which surfaces coupons
    navigation.getParent()?.navigate('Deals');
  };
  ...
  const ListHeader = (
    <View>
      ...
      {searchQuery.trim() === '' && (
        <>
          {/* Hero Carousel from top coupons */}
          <HeroBanner coupons={coupons.slice(0, 5)} />

          {/* Platform cards */}
          <PlatformCards platforms={platforms} onShopNow={handlePlatformShopNow} />
          ...
          {categories.slice(0, 3).map((cat) => (
            <CashbackSection key={cat} category={cat} platforms={catPlatformMap[cat] ?? []} />
          ))}

          {/* Top Deals horizontal */}
          <TopDealsRow products={featured} onPress={goToDetail} />
        </>
      )}

      {/* Products grid title */}
      <Text style={{ fontSize: 17, fontWeight: '700', color: '#111827', paddingHorizontal: 16, marginBottom: 12 }}>
        {searchQuery.trim() ? `Results for "${searchQuery.trim()}"` : (selectedCategory ?? 'All Products')}
